/**
 * Live 4-browser end-to-end test: Chrome + Firefox + Chromium + Chromium
 * Plays a full 4-PLAYER game of Thousand: lobby → bidding → card exchange → tricks → victory
 *
 * Usage:  node tests/e2e-live-4p.js
 */
...
async function main() {
  ...
  try {
    ...
    console.log('Launching Chrome, Firefox, Chromium, Chromium…');
    const chromiumArgs = [
    ...
    const chromiumBrowser2 = await chromium.launch({                       headless: HEADLESS, slowMo: SLOW_MO, args: chromiumArgs });
    browsers.push(chromeBrowser, firefoxBrowser, chromiumBrowser, chromiumBrowser2);

    const alice   = await chromeBrowser.newPage();
    const bob     = await firefoxBrowser.newPage();
    const charlie = await chromiumBrowser.newPage();
    const dave    = await chromiumBrowser2.newPage();
    ...
    // ── Navigate ──────────────────────────────────────────────────────────────
    await Promise.all([alice, bob, charlie, dave].map(p => p.goto(BASE_URL)));
    console.log('All four browsers at', BASE_URL, '\n');
    ...
    log('Dave',    'entering nickname…');
    await dave.fill('#nickname-input', 'Dave');
    await dave.click('#nickname-form button[type="submit"]');
    ...
    // "public" radio is checked by default — select the 4-player option, then submit
    await alice.check('input[name="player-count"][value="4"]');
    await alice.click('#new-game-form button[type="submit"]');
    log('Alice', 'created 4-player game');
    ...
    await charlie.waitForSelector('#game-screen:not(.hidden)', { timeout: 8000 });
    log('Charlie', 'joined & in waiting room');

    // ── Dave joins ────────────────────────────────────────────────────────────
    await dave.waitForSelector('#lobby-screen:not(.hidden)', { timeout: 8000 });
    await dave.waitForSelector('#game-list li[data-id]', { timeout: 10000 });
    await dave.click('#game-list li[data-id]');
    await dave.click('#join-selected-btn');
    log('Dave', 'joined');

    // ── Wait for the round to auto-start ─────────────────────────────────────
    // When the 4th player joins the server broadcasts round_started and
    // all clients transition to the round screen (bid controls appear).
    console.log('\n— All four joined. Waiting for round to start… —\n');
    ...
    const players = [
      ...
      { page: dave,    name: 'Dave',    stats: { exchangePassesByMe: 0, lastHandSizeDuringExchange: null, handGrowthObserved: false } },
    ];
    ...
    if (doneCnt === 4) {
      console.log('\n✅  FULL GAME COMPLETE — all 4 players back in lobby!\n');
    } else {
      console.log(`\n⚠️  Loop exited after ${iter} iterations. ${doneCnt}/4 players finished.\n`);
