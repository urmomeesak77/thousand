...
description: "Task list for feature 005 â€” Play Phase, Scoring, Multi-Round & Victory"
---
...
**Tests**: Test tasks are included â€” the plan and quickstart explicitly enumerate the test files for this feature.

**Organization**: Tasks are grouped by user story (P1 â†’ P4) to enable independent implementation and testing. Each phase corresponds to one merge-ready PR per plan.md "Implementation Phases (delivery order)".
...
- [x] T002 [P] Add `CARD_POINT_VALUE` (A=11, 10=10, K=4, Q=3, J=2, 9=0), `MARRIAGE_BONUS` (â™£=100, â™ =80, â™¥=60, â™¦=40), and `RANK_ORDER` ({'9':0,'J':1,'Q':2,'K':3,'10':4,'A':5}) constants to `src/public/js/thousand/constants.js` (client-side) per FR-008 / FR-013 / Decision 4
- [x] T003 [P] Add server-side `CARD_POINT_VALUE`, `MARRIAGE_BONUS`, `RANK_ORDER` constants exported from `src/services/Scoring.js` (new file scaffold â€” pure-function module; method bodies filled in US1/US2/US3) per Decision 3 / Decision 4
...
## Phase 3: User Story 1 â€” Card Exchange, Trick Play, Round Summary (Priority: P1) ðŸŽ¯ MVP

**Goal**: Replace feature 004's "Round ready to play" handoff with the end-to-end gameplay loop for a single round â€” declarer exchange â†’ 8-trick play (no trump, no marriages) â†’ made/missed summary â†’ Back to Lobby.
...
- [x] T008 [P] [US1] Write `tests/Round.trickplay.test.js` covering FR-006 / FR-007 follow-suit and FR-008 winner determination â€” include a table-driven "Ten beats King and Queen, Ace beats Ten" check per R-003
...
- [x] T013 [P] [US1] Write `tests/round-messages.005.test.js` covering end-to-end `exchange_pass` â†’ `play_card` Ã— 24 â†’ `round_summary` via `ConnectionManager`
...
- [x] T015 [P] [US1] Implement `roundDeltas(roundScores, declarerSeat, bid, penalties = [])` in `src/services/Scoring.js` per FR-014 â€” declarer `+bid` / `âˆ’bid`; opponent `+roundScore`; penalties param is plumbing for US4
- [x] T016 [US1] Create `src/services/TrickPlay.js` with `initialize(round, declarerSeat)`, `playCard(seat, cardId)` (follow-suit-aware), `resolveTrick()` per FR-006 / FR-007 first clause / FR-008 (no trump path yet) â€” owns `currentTrick`, `trickNumber`, `currentTrickLeaderSeat`, `collectedTricks`, `collectedTrickCounts`
- [x] T017 [US1] Add `submitExchangePass(seat, cardId, destSeat)` method to `src/services/Round.js` per FR-002 / FR-003 â€” validates card-in-hand + destination + non-duplicate, mutates `hands`, increments `exchangePassesCommitted`, transitions to `trick-play` on the second commit
...
- [x] T019 [US1] Add `Round.buildSummary()` to `src/services/Round.js` per FR-015 â€” assembles `RoundSummary` view-model (no marriage bonus column populated yet; `victoryReached` always `false` in this milestone)
- [x] T020 [US1] Extend `src/services/RoundPhases.js` with `post-bid-decision â†’ card-exchange` transition (replaces FR-001 superseded `play-phase-ready` path) and `card-exchange â†’ trick-play â†’ round-summary` transitions
...
- [x] T026 [US1] Add `exchange_pass` message branch to `src/controllers/RoundActionHandler.js` â€” throttle check, seat-is-declarer check, calls `Round.submitExchangePass`, broadcasts `card_passed` (per-viewer identity filter per FR-019) and `phase_changed`
- [x] T027 [US1] Add `play_card` message branch (no `declareMarriage` yet) to `src/controllers/RoundActionHandler.js` â€” throttle, `seat === currentTurnSeat` check, calls `Round.playCard`, broadcasts `card_played`, on trick-fill broadcasts `trick_resolved`, on round-end runs `Scoring.roundScores` + `Scoring.roundDeltas` and broadcasts `round_summary`
...
- [x] T029 [P] [US1] Create `src/public/js/thousand/CardExchangeView.js` â€” declarer 10-card hand with tap-to-select then two destination buttons; opponent waiting state per FR-002 / FR-020 (hidden for opponents)
- [x] T030 [P] [US1] Create `src/public/js/thousand/TrickPlayView.js` â€” centre slot, lead/follow prompt, hand with FR-007 follow-suit pre-disable, mounts the per-seat `CollectedTricksStack` widgets per FR-020
- [x] T031 [P] [US1] Create `src/public/js/thousand/CollectedTricksStack.js` â€” per-seat face-down stack + `Ã— N` count badge per FR-008
- [x] T032 [P] [US1] Create `src/public/js/thousand/RoundSummaryScreen.js` â€” per-player rows (nickname, trick points, round total, delta, cumulativeAfter), declarer made/missed indicator, single Back-to-Lobby button per FR-015 (Continue button lands in US3)
...
- [x] T036 [US1] Add `sendExchangePass(cardId, toSeat)` and `sendPlayCard(cardId, opts = {})` outbound wrappers to `src/public/js/thousand/RoundActionDispatcher.js` â€” consults the `inFlightAnimation` gate per FR-030 / R-008
...
- [x] T038 [US1] Implement the three card-motion animations (250 ms handâ†’recipient on exchange; 250 ms handâ†’centre on play; 350 ms pause + 250 ms centreâ†’winner-stack on resolve) via `Antlion.onTick` and `Antlion.schedule` in `src/public/js/thousand/CardExchangeView.js` / `TrickPlayView.js` per FR-030 / Decision 10
...
## Phase 4: User Story 2 â€” Marriages and Trump Declaration (Priority: P2)

**Goal**: Add the marriage-declaration prompt on tricks 2â€“6, the combined `play_card { declareMarriage }` flow, the trump-suit state machine, and the trump-priority extension to follow-suit (FR-007 second clause).

**Independent Test**: From a round where the declarer holds Kâ™¥ and Qâ™¥, the declarer leads trick 2 by tapping Kâ™¥, confirms "Declare marriage in Hearts (+60)". The status bar shows "Trump: â™¥"; the declarer's round score is +60 before any trick points. On a later trick where the led suit is Diamonds and the declarer is out of Diamonds, they play 9â™¥ (a trump) and 9â™¥ wins the trick over any non-trump.
...
## Phase 5: User Story 3 â€” Multi-Round, Dealer Rotation, Victory at 1000+ (Priority: P3)
...
**Independent Test**: Three players play through a sequence of rounds. After each round summary, all three press Continue; a new round begins with the dealer rotated clockwise. Cumulative scores carry across rounds and are visible at all times. When any player's cumulative reaches â‰¥ 1000, the game ends; all three clients show the final-results screen with the winner highlighted and a per-round history table.
...
## Phase 6: User Story 4 â€” Special Scoring: Barrel and Three Consecutive Zeros (Priority: P4)

**Goal**: Add the barrel rule (cumulative in [880, 1000) â‡’ 120-bid floor + 3-round counter + âˆ’120 penalty) and the three-consecutive-zeros rule (âˆ’120 penalty on the 3rd consecutive zero round).

**Independent Test**: Drive a player's cumulative to 895; on the next round the status display shows "On barrel â€” round 1 of 3". The server rejects bids below 120 from that player. After 3 on-barrel rounds without reaching 1000, a âˆ’120 penalty is applied. Separately, drive a player to 3 consecutive rounds with round score exactly 0; the third triggers a âˆ’120 penalty.
...
- [x] T092 [US4] Extend `src/public/js/thousand/StatusBar.js` with the `barrelMarkers` indicator (“On barrel â€” round N of 3”) next to the matching seat label per FR-018
...
- [ ] T095 [US4] Extend `src/public/js/thousand/RoundSummaryScreen.js` to render the "Barrel penalty: âˆ’120" and "Zero-round penalty: âˆ’120" rows as separate line items on the affected player's column per FR-023 / FR-024
...
- **Phase 3 (US1, P1)** â€” depends on Phase 2. MVP slice; can ship standalone (Back-to-Lobby summary, no multi-round).
- **Phase 4 (US2, P2)** â€” depends on US1 (extends `TrickPlay`, `Scoring.roundScores`, `Round.buildSummary`, `TrickPlayView`, `StatusBar`).
- **Phase 5 (US3, P3)** â€” depends on US1 (replaces US1's temporary round-end purge; extends the round summary screen). Does **not** depend on US2 â€” multi-round is orthogonal to marriages.
- **Phase 6 (US4, P4)** â€” depends on US3 (operates on the persistent `Game` entity and its `applyRoundEnd` hook).
- **Phase 7 (Polish)** â€” depends on whichever stories are complete in this PR.

### Within Each User Story

- Tests first â†’ make tests fail meaningfully â†’ then implementation in this order: pure functions â†’ services â†’ controllers â†’ frontend components â†’ wiring & animation â†’ CSS.
- Server-side message-handler changes (e.g., T025â€“T028 in US1) require the corresponding service methods (T014â€“T019) to exist.
- Client-side message handlers (e.g., T037 in US1) require the outbound dispatcher wrappers (T036) and the screens (T029â€“T032) to exist.

### Parallel Opportunities

- **Phase 2**: T002 / T003 / T005 can run in parallel (different files).
- **Phase 3 tests**: T006â€“T013 are all `[P]` (different test files).
...
Task: "Write tests/Scoring.test.js â€” FR-013 / FR-014 (T006)"
Task: "Write tests/Round.cardexchange.test.js â€” FR-002 / FR-003 (T007)"
Task: "Write tests/Round.trickplay.test.js â€” FR-006 / FR-007 / FR-008 (T008)"
Task: "Write tests/CardExchangeView.test.js â€” FR-002 (T009)"
Task: "Write tests/TrickPlayView.test.js â€” FR-007 / FR-008 (T010)"
Task: "Write tests/RoundSummaryScreen.test.js â€” FR-015 (T011)"
Task: "Write tests/StatusBar.005.test.js â€” FR-018 (T012)"
Task: "Write tests/round-messages.005.test.js â€” end-to-end (T013)"
...
2. US1 â†’ ship (single-round MVP).
3. US2 â†’ ship (marriages + trump add the strategic core; bidding economy starts working).
4. US3 â†’ ship (multi-round + victory; the implementation is now a full game).
5. US4 â†’ ship (barrel + three-zeros; rulebook coverage complete).
...
- Every test file referenced in `quickstart.md` Â§ "Running tests" appears as a task in this list (FR coverage is enforceable).
- Animation timing values (250 / 350 / 250 ms) are from research.md Decision 10 â€” these are the targets, not contracts; the timing constants live in `src/public/js/thousand/constants.js` for easy tuning.
- The `play_phase_ready` message remains in the protocol for the abort path (`round_aborted`) only â€” see research.md Decision 8.
