...
- [ ] T015 [P] [US2] Test pure helpers in `tests/botStrategyHelpers.test.js`: `rankStrength`, `cardBeats`, `bestCenterCard`, `estimateMakeable`, `findMarriages`, `pickCard` (ported from `tests/e2e-live-smart.js`) (per FR-007, FR-008)
...
- [ ] T022 [US2] Integration test in `tests/bots-autoplay.integration.test.js`: one human + bots fill a 3-player table; drive timers (fake/advanced) and assert the round advances bidding → exchange → trick-play → round-summary with no human input and produces valid scores (per FR-006, FR-007, FR-010, SC-001)
