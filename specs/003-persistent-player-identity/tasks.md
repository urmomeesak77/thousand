...
- [ ] T020 [US2] Add `_purgePlayer(playerId)` private method to `ThousandStore` in `src/services/ThousandStore.js` — contains the delete-and-notify logic currently in `handlePlayerDisconnect` (delete from map, resolve game exit via `_resolveGameAfterExit`). The existing `_resolveGameAfterExit` → `broadcastLobbyUpdate()` path is what makes remaining lobby members see the removal — satisfies FR-006 (lobby player list updates passively, no notification banner).
- [ ] T021 [US2] Add `_gracePeriodMs` to `ThousandStore` constructor in `src/services/ThousandStore.js` — default `process.env.GRACE_PERIOD_MS ? Number(process.env.GRACE_PERIOD_MS) : 30_000`. Must precede T022 (timer references this field).
- [ ] T022 [US2] Rewrite `ThousandStore.handlePlayerDisconnect(playerId)` in `src/services/ThousandStore.js` — early-return if `!playerId || !this.players.has(playerId)` (handles pre-hello disconnects passed `undefined` from T009); otherwise set `player.ws = null`, `player.disconnectedAt = Date.now()`; start `setTimeout(() => this._purgePlayer(playerId), this._gracePeriodMs)` stored as `player.graceTimer`; do NOT delete the record immediately
- [ ] T023 [US2] Add `reconnectPlayer(playerId, ws)` to `ThousandStore` in `src/services/ThousandStore.js` — cancel `player.graceTimer` via `clearTimeout`, reset `disconnectedAt` to null, assign new `ws`, and set `ws._playerId = player.id` (required by the close handler in T009 to identify this player on future disconnect); if `player.ws` was already open (readyState OPEN), send `{ type: 'session_replaced' }` and close it first (last-connect-wins)
- [ ] T024 [US2] Update `ConnectionManager._handleMessage` hello branch in `src/services/ConnectionManager.js` — when `result.restored === true`, call `this._store.reconnectPlayer(result.playerId, ws)` (T023) to attach the new ws and cancel any grace timer; then send `connected`
...
- [ ] T026 [US2] Handle `session_replaced` in `ThousandApp._handleMessage` in `src/public/js/ThousandApp.js` — show toast "Connected from another tab or browser — this session ended." (covers both same-browser multi-tab per Edge Cases and cross-browser transfer per US3 acceptance scenario 2).
