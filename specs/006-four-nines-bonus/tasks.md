...
- [ ] T028 Execute the `specs/006-four-nines-bonus/quickstart.md` 3-tab manual walkthrough (force the four-nines hand via the test-deck seam), including the reconnect-mid-gate and premature-lead spot-checks
  - **Outstanding**: requires a live 3-browser session; not runnable in the headless implementation environment. The `round-messages.fournines` integration test exercises the same exchange→award→ack→lead→completion path end-to-end via `ConnectionManager`.
