...
src/public/
  ...
  js/
    ...
    i18n/                              # internationalization layer (feature 013)
      I18n.js                          # current language + `t(key, params)`; en-catalog fallback (FR-009), Intl.PluralRules-driven plural selection (FR-010), {token} interpolation; emits `language:changed`
      LanguagePreferenceStore.js       # localStorage mirror of the language choice (`thousand_lang`); invalid/unreadable value → null (→ detection)
      LanguageButton.js                # binds every `.lang-btn`, toggles en↔ru, shows the target language (mirrors MuteButton)
      PageTranslator.js                # applies `data-i18n` / `data-i18n-attr` to static HTML; re-walks on `language:changed`
      catalogs/en.js                   # English catalog — source of truth + universal fallback (every key defined here)
      catalogs/ru.js                   # Russian catalog — mirrors every en key (parity enforced by tests/catalogParity.test.js); ru plural objects { one, few, many }
...
- **013 multilanguage-support** — client-side English/Russian i18n (`src/public/js/i18n/`). An `I18n` service resolves dot-namespaced keys against static per-language catalogs (`en.js` = source of truth + fallback, `ru.js` auto-translated with Тысяча terminology), with `Intl.PluralRules`-driven Russian plural forms (FR-010) and `{token}` interpolation; English is the universal fallback so `t()` never returns empty (FR-009). Static HTML carries `data-i18n`/`data-i18n-attr` applied by `PageTranslator`; dynamic component strings and the pure formatters (`statusText`, `historyEntryText`, `roundStatsText`) take a bound `t` as their first argument (`GameScreen`/`ThousandApp` thread it down like `Toast`). The language toggle (`LanguageButton`, every `.lang-btn` in the lobby header + scoreboard icon row) flips en↔ru, persists via `LanguagePreferenceStore` (`thousand_lang`, default by browser language on first visit, FR-008), and emits `language:changed` — subscribers (`GameScreen`, `HistoryPanel`, `ScoreboardPanel`, `ThousandApp`, `PageTranslator`, `MuteButton`) re-render every visible label from retained state with no reload and no game-state change (FR-005/FR-011). The thin server stays language-agnostic: action-rejection/`game_join_failed`/`round_aborted` payloads gain a stable `code` (+ primitive `params`) beside the unchanged English `reason` (the `reject.*` keyset of `en.js` IS the code registry, guarded by `tests/rejection-codes.test.js`); `ThousandMessageRouter` words them via `i18n.t(code, params)` with the English `reason` as fallback. Infrastructure `error` payloads stay English (exempt). `tests/catalogParity.test.js` enforces full en↔ru key parity.
